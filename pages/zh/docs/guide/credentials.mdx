...
`loader.makeServerCredentials()` 与 `loader.makeClientCredentials()` 覆盖了
常用场景。如果你需要更精细的控制 —— 例如调用 `createFromSecureContext`、
搭建 Google ADC 链路或组合 credentials，gRPCity 也把 `credentials` 命名
空间和 `ChannelCredentials` / `ServerCredentials` 类型从主包里重新导出，
免去对 `@grpc/grpc-js` 的直接依赖：
