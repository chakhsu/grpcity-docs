...
每次 gRPC 调用都会以一个 `status` 结尾。`gRPCity` 把数值与符号映射做得很易用。这页平时不太会读，但日志里出现某个错误码时，你会回来查它。

### 读取 status

数值与符号的映射挂在包对象的 `Status` 导出上：
...
调用结束后，客户端会和 response 一起返回完整的 status：
...
除了 `OK` 之外，下表把最常见的几种情况映射到对应的 code 与触发方：
