...
客户端发送 `metadata` 通常会用 `loader.makeMetadata()`（每个 loader 自带的
辅助函数）。如果你更喜欢直接构造 `Metadata` 实例，gRPCity 也把 `Metadata`
类从主包重新导出，免得再去依赖 `@grpc/grpc-js`：
...
下面使用 loader helper（其实只是返回一个 `Metadata` 实例的薄封装）：
