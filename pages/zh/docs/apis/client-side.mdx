...
`loader.initClients()` 返回的 `clients` 工厂以及它派生的每个方法代理的参考。下面分两层：工厂本身的 `get` / `use` / `clear` / `init`，以及四类 RPC（unary、client-stream、server-stream、bidi）的调用形态。
...
<OptionTable
  options={[
    ...
    ['options', 'Object', '可选，per-call 选项对象。可用字段包括 `timeout`（单次调用超时，默认 10 秒）以及 `signal`（用于取消调用的 `AbortSignal`，详见 [AbortSignal](../guide/abort-signal)）']
  ]}
/>
