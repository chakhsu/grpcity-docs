...
`loader.initServer()` 返回的 `server` 实例的参考。下面的方法覆盖完整生命周期：注册/注销 service、添加中间件、监听、退出。
...
<OptionTable
  options={[
    ['customService', 'Object', '一个预先构造好的 service 对象，需要带有 inject(server) 方法。loader.initReflection() 返回的就是这种对象。']
  ]}
/>
