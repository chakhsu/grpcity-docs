...
`channelOptions` are explained in detail in the [Config guide](/guide/config).
...
A distinctive feature of `gRPCity` Server is that it provides middleware capabilities for pre- and post-processing of `implementation`. For more details, see the [Middleware guide](/guide/middleware).
...
For more information, you can read the [Credentials guide](/guide/credentials).
