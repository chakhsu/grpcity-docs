...
protobuf 文件加载官方提供了两种方式，一种是直接加载 pb 文件，另外一种是工具编译后再通过依赖引用的方式。这两种方式的各有优劣，在这里 gRPCity 最终选择了仅支持第一种方式，就是直接加载 pb 文件。

下面我们将会通过新建项目的方式进行快速入门。
...
至此，我们完成了 proto 文件的加载工作，并得到了`loader.js`，这样我们就可以随处引入该文件，并支持我们去开发客户端调用或者服务端启动。
