...
To send `metadata` from the client, use `loader.makeMetadata()` (the
helper that ships with every loader). If you'd rather construct a
`Metadata` instance directly, gRPCity also re-exports the class so you
don't have to depend on `@grpc/grpc-js`:
...
Below uses the loader helper, which is just a thin wrapper that returns
a `Metadata` instance:
