...
`channelOptions` are passed straight through to `@grpc/grpc-js`. The full list of channel arguments is documented [here](https://grpc.github.io/grpc/core/group__grpc__arg__keys.html); the subset that grpc-js currently honours is documented [here](https://github.com/grpc/grpc-node/tree/master/packages/grpc-js#supported-channel-options).
