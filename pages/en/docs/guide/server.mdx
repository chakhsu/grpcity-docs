...
Just `throw`. gRPCity catches the thrown `Error` and turns it into a gRPC
status on the wire for any of the four RPC kinds (unary, client stream,
server stream, bidi).

For server-stream and bidi handlers in particular, throwing is the
correct way to fail — gRPCity routes the error through the underlying
server stream's `'error'` channel so the client receives the status
trailer immediately rather than waiting for its deadline. See the
[Error guide](./error) for what the client sees.
