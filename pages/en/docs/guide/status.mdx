...
Every gRPC call ends with a `status`. `gRPCity` makes the values and the numeric mapping easy to access. You won't read this page often, but it's the reference you'll come back to when an error code shows up in logs.

### Reading the status

The numeric/symbolic status enum lives on the package's `Status` export:
...
On a completed call, the client returns the full status alongside the response:
...
### When each code appears

Apart from `OK`, the table below maps the most common situations to the code you'll see and which side raised it:
