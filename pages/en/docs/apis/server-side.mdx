...
Reference for the `server` instance returned by `loader.initServer()`. The methods below cover the full server lifecycle: register and unregister services, add middleware, listen, and shut down.
...
<OptionTable
  options={[
    ['customService', 'Object', 'A pre-built service object exposing an `inject(server)` method. `loader.initReflection()` returns one.']
  ]}
/>
