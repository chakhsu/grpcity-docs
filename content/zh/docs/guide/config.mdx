...
- `channelOptions` —— 底层 gRPC channel 在网络层的行为。在 `initClients()` 与 `initServer()` 时传入。
...
`channelOptions` 原样透传给 `@grpc/grpc-js`。所有 channel 参数见 [这里](https://grpc.github.io/grpc/core/group__grpc__arg__keys.html)；grpc-js 当前实际支持的子集见 [这里](https://github.com/grpc/grpc-node/tree/master/packages/grpc-js#supported-channel-options)。
