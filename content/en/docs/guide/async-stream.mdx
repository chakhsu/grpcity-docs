...
This page walks through `gRPCity`'s recommended streaming API: promise-based `for await` consumption on the client and the server. If you need the older callback form, see the [Stream](/en/docs/guide/stream) guide first — the two pages share the proto and most of the setup.

### Define the proto

The proto declares the three streaming RPC kinds — client-stream, server-stream, and bidi:
